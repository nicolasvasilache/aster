...
// CHECK-NEXT:    %{{.*}} = load global_load_dword dest %[[R13]] addr %[[R11]] : dps(!amdgcn.vgpr_range<[12 : 13]>) ins(!amdgcn.vgpr_range<[10 : 12]>) -> !amdgcn.read_token<flat>
// CHECK-NEXT:    %{{.*}} = load s_load_dwordx2 dest %{{.*}} addr %{{.*}} : dps(!amdgcn.sgpr_range<[2 : 4]>) ins(!amdgcn.sgpr_range<[0 : 2]>) -> !amdgcn.read_token<constant>
...
// CHECK-NEXT:    amdgcn.vop1.vop1 <v_mov_b32_e32> %[[R7]], %{{.*}} : (!amdgcn.vgpr<10>, !amdgcn.sgpr<2>) -> ()
// CHECK-NEXT:    amdgcn.vop1.vop1 <v_mov_b32_e32> %[[R8]], %{{.*}} : (!amdgcn.vgpr<11>, !amdgcn.sgpr<3>) -> ()
// CHECK-NEXT:    store global_store_dword data %[[R13]] addr %[[R11]] : ins(!amdgcn.vgpr_range<[12 : 13]>, !amdgcn.vgpr_range<[10 : 12]>) -> !amdgcn.write_token<flat>
// CHECK-NEXT:    end_kernel
  kernel @gpu_copy_kernel_unchecked {
    ...
    %t1 = amdgcn.load s_load_dwordx2 dest %5 addr %2 : dps(!amdgcn.sgpr_range<[2 : 4]>) ins(!amdgcn.sgpr_range<[0 : 2]>) -> !amdgcn.read_token<constant>
    ...
    %t2 = amdgcn.load global_load_dword dest %13 addr %11 : dps(!amdgcn.vgpr_range<[12 : 13]>) ins(!amdgcn.vgpr_range<[10 : 12]>) -> !amdgcn.read_token<flat>
    ...
    %t3 = amdgcn.load s_load_dwordx2 dest %20 addr %17 : dps(!amdgcn.sgpr_range<[2 : 4]>) ins(!amdgcn.sgpr_range<[0 : 2]>) -> !amdgcn.read_token<constant>
    amdgcn.sopp.s_waitcnt <s_waitcnt> vmcnt = 0
    ...
    %t4 = amdgcn.store global_store_dword data %13 addr %26 : ins(!amdgcn.vgpr_range<[12 : 13]>, !amdgcn.vgpr_range<[10 : 12]>) -> !amdgcn.write_token<flat>
    end_kernel
  }
}
