...
// CHECK:       Op: %[[R_0:.*]], %[[R_1:.*]] = amdgcn.load global_load_dword dest %[[R_2:.*]] addr %[[R_3:.*]] : dps(!amdgcn.vgpr_range<[? + 1]>) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       Op: %[[R_4:.*]] = amdgcn.store global_store_dword data %[[R_0]] addr %[[R_3]] : ins(!amdgcn.vgpr_range<[? + 1]>, !amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.write_token<flat>
...
// CHECK:       Op: func.func @test_pipelined_pattern(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_1:.*]], %[[R_2:.*]] = amdgcn.load global_load_dword dest %[[R_3:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_2]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_4:.*]], %[[R_5:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: %[[R_6:.*]], %[[R_7:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: %[[R_19:.*]], %[[R_20:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: func.func @test_escaped_waits_1(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: scf.for %[[R_1:.*]] = %[[R_2:.*]] to %[[R_3:.*]] step %[[R_4:.*]] {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{<escaped>, 0, flat}]
// CHECK:       Op: %[[R_5:.*]], %[[R_6:.*]] = amdgcn.load global_load_dword dest %[[R_7:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{<escaped>, 0, flat}]
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{<escaped>, 0, flat}]
func.func @test_escaped_waits_1(%arg0: !amdgcn.vgpr_range<[? + 2]>) {
  %0 = amdgcn.alloca : !amdgcn.vgpr
  %1 = amdgcn.alloca : !amdgcn.vgpr
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
...
// CHECK:       Op: func.func @test_escaped_waits_2(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>, %[[R_1:.*]]: i1) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: scf.if %[[R_1]] {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{<escaped>, 0, flat}]
// CHECK:       Op: %[[R_2:.*]], %[[R_3:.*]] = amdgcn.load global_load_dword dest %[[R_4:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       	WAIT STATE AFTER: <Empty>
func.func @test_escaped_waits_2(%arg0: !amdgcn.vgpr_range<[? + 2]>, %arg1: i1) {
  %0 = amdgcn.alloca : !amdgcn.vgpr
  %1 = amdgcn.alloca : !amdgcn.vgpr
  scf.if %arg1 {
    %result, %token = amdgcn.load global_load_dword dest %0 addr %arg0 : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: func.func @test_if_flow_1(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>, %[[R_1:.*]]: i1) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_2:.*]] = scf.if %[[R_1]] -> (!amdgcn.read_token<flat>) {...} else {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_2]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_3:.*]], %[[R_4:.*]] = amdgcn.load global_load_dword dest %[[R_5:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: scf.yield %[[R_4]] : !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_3]], %[[R_4]] = amdgcn.load global_load_dword dest %[[R_6:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: scf.yield %[[R_4]] : !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
...
// CHECK:       Op: func.func @test_if_flow_2(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>, %[[R_1:.*]]: i1) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_2:.*]] = scf.if %[[R_1]] -> (!amdgcn.read_token<flat>) {...} else {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_6:.*]], %[[R_7:.*]] = amdgcn.load global_load_dword dest %[[R_5]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 1, flat}, {%[[R_7]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_8:.*]], %[[R_9:.*]] = amdgcn.load global_load_dword dest %[[R_5]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 2, flat}, {%[[R_7]], {{[0-9]*}}, 1, flat}, {%[[R_9]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: %[[R_3]], %[[R_4]] = amdgcn.load global_load_dword dest %[[R_10:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
// CHECK:       Op: scf.yield %[[R_4]] : !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 0, flat}]
...
// CHECK:       Op: func.func @test_if_flow_3(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>, %[[R_1:.*]]: i1) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_2:.*]] = scf.if %[[R_1]] -> (!amdgcn.read_token<flat>) {...} else {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       Op: func.func @test_passthrough_pattern(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_1:.*]], %[[R_2:.*]] = amdgcn.load global_load_dword dest %[[R_3:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       Op: func.func @test_mixed_smem_dsmem(%[[R_0:.*]]: !amdgcn.sgpr_range<[? + 2]>, %[[R_1:.*]]: !amdgcn.vgpr, %[[R_2:.*]]: !amdgcn.vgpr_range<[? + 2]>) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_3:.*]], %[[R_4:.*]] = amdgcn.load s_load_dword dest %[[R_5:.*]] addr %[[R_0]] : dps(!amdgcn.sgpr) ins(!amdgcn.sgpr_range<[? + 2]>) -> !amdgcn.read_token<constant>
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       Op: %[[R_9:.*]], %[[R_10:.*]] = amdgcn.load global_load_dword dest %[[R_8]] addr %[[R_2]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_4]], {{[0-9]*}}, 1, constant}, {%[[R_7]], {{[0-9]*}}, 0, shared}]
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_10]], {{[0-9]*}}, 0, flat}, {%[[R_4]], {{[0-9]*}}, 2, constant}, {%[[R_7]], {{[0-9]*}}, 1, shared}]
// CHECK:       Op: amdgcn.wait deps %[[R_4]] : !amdgcn.read_token<constant>
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_10]], {{[0-9]*}}, 0, flat}, {%[[R_4]], {{[0-9]*}}, 2, constant}, {%[[R_7]], {{[0-9]*}}, 1, shared}]
// CHECK:       	WAIT STATE AFTER: unhandled tokens = [{%[[R_10]], {{[0-9]*}}, 0, flat}], wait information = {counts: {vm_cnt: nowait, lgkm_cnt: 0}, waited_tokens: [], implied_tokens: [{%[[R_4]], {{[0-9]*}}, 2, constant}, {%[[R_7]], {{[0-9]*}}, 1, shared}]}
// CHECK:       Op: func.return
// CHECK:       	WAIT STATE BEFORE: unhandled tokens = [{%[[R_10]], {{[0-9]*}}, 0, flat}], wait information = {counts: {vm_cnt: nowait, lgkm_cnt: 0}, waited_tokens: [], implied_tokens: [{%[[R_4]], {{[0-9]*}}, 2, constant}, {%[[R_7]], {{[0-9]*}}, 1, shared}]}
...
// CHECK:       Op: func.func @test_counts_strength(%[[R_0:.*]]: !amdgcn.vgpr_range<[? + 2]>) {...}
// CHECK:       	WAIT STATE BEFORE: <Empty>
// CHECK:       	WAIT STATE AFTER: <Empty>
// CHECK:       Op: %[[R_1:.*]], %[[R_2:.*]] = amdgcn.load global_load_dword dest %[[R_3:.*]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
// CHECK:       	WAIT STATE BEFORE: <Empty>
...
// CHECK:       Op: %[[R_8:.*]], %[[R_9:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: %[[R_10:.*]], %[[R_11:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
...
// CHECK:       Op: %[[R_12:.*]], %[[R_13:.*]] = amdgcn.load global_load_dword dest %[[R_3]] addr %[[R_0]] : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr_range<[? + 2]>) -> !amdgcn.read_token<flat>
