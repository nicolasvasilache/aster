...
amdgcn.module @test_memory_dependence target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  amdgcn.kernel @test_kernel {
    ...
    %6, %tok1 = amdgcn.load global_load_dword dest %dst_range0 addr %addr_range offset c(%c0) { test.load_tag_0 } : dps(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.read_token<flat>

    // CHECK: Operation: {{.*}}global_load{{.*}}test.load_tag_1
    // CHECK-NEXT: PENDING BEFORE: 1: test.load_tag_0
    // CHECK-NEXT: MUST FLUSH NOW: 0:
    %7, %tok2 = amdgcn.load global_load_dword dest %dst_range1 addr %addr_range offset c(%c4) { test.load_tag_1 } : dps(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.read_token<flat>

    // CHECK: Operation: {{.*}}global_load{{.*}}test.load_tag_2
    // CHECK-NEXT: PENDING BEFORE: 2: test.load_tag_0, test.load_tag_1
    // CHECK-NEXT: MUST FLUSH NOW: 0:
    %8, %tok3 = amdgcn.load global_load_dword dest %dst_range2 addr %addr_range offset c(%c8) { test.load_tag_2 } : dps(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.read_token<flat>
    ...
    %tok4 = amdgcn.store global_store_dword data %data_range addr %addr_range offset c(%c12) { test.store_tag_0 } : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.write_token<flat>
    ...
    // CHECK-NEXT: MUST FLUSH NOW: 0:
    %tok5 = amdgcn.store global_store_dword data %data_range addr %addr_range offset c(%c16) { test.store_tag_1 } : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.write_token<flat>
    ...
    %ds_14, %tok_ds2 = amdgcn.load ds_read_b32 dest %ds_dst_range addr %13 offset c(%c12_ds_mem) { test.ds_read_tag_0 }
      : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr, i32) -> !amdgcn.read_token<shared>
    ...
    %10, %tok6 = amdgcn.load global_load_dwordx2 dest %dst_range3 addr %addr_range offset c(%c16) { test.load_tag_3 } : dps(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.read_token<flat>
    ...
    %tok7 = amdgcn.store global_store_dword data %ds_14 addr %addr_range offset c(%c12) { test.global_store_tag_0 } : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.write_token<flat>
  ...
  amdgcn.kernel @test_noalias_pointers {
    ...
    // CHECK-NEXT: PENDING BEFORE: 0:
    // CHECK-NEXT: MUST FLUSH NOW: 0:
    %tok_na1 = amdgcn.store global_store_dword data %data_range addr %ptr1_noalias offset c(%c0_na) { test.store_noalias_tag_0 } : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.write_token<flat>
    ...
    // CHECK-NEXT: MUST FLUSH NOW: 0:
    %loaded, %tok_na2 = amdgcn.load global_load_dword dest %dst_range addr %ptr2_noalias offset c(%c0_na) { test.load_noalias_tag_0 } : dps(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? + 2]>, i32) -> !amdgcn.read_token<flat>
