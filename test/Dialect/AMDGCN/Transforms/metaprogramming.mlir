...
func.func @test_interleaving() -> !amdgcn.vgpr_range<[? + 4]> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %c5 = arith.constant 5 : index
  %alloca = memref.alloca() : memref<!amdgcn.vgpr_range<[? + 4]>>
  %alloca_0 = memref.alloca() : memref<!amdgcn.vgpr_range<[? + 4]>>
  scf.for %arg0 = %c0 to %c5 step %c1 {
    %1 = arith.remsi %arg0, %c2 : index
    %2 = arith.cmpi eq, %1, %c0 : index
    %3:2 = scf.if %2 -> (memref<!amdgcn.vgpr_range<[? + 4]>>, memref<!amdgcn.vgpr_range<[? + 4]>>) {
      scf.yield %alloca, %alloca_0 : memref<!amdgcn.vgpr_range<[? + 4]>>, memref<!amdgcn.vgpr_range<[? + 4]>>
    } else {
      scf.yield %alloca_0, %alloca : memref<!amdgcn.vgpr_range<[? + 4]>>, memref<!amdgcn.vgpr_range<[? + 4]>>
    }
    %4 = memref.load %3#0[] : memref<!amdgcn.vgpr_range<[? + 4]>>
...
// CHECK-LABEL:   func.func @test_interleaving() -> !amdgcn.vgpr_range<[? + 4]> {
...
// CHECK:           %[[TEST_INST_0:.*]] = amdgcn.test_inst outs %[[MAKE_REGISTER_RANGE_0]] ins %[[MAKE_REGISTER_RANGE_1]] : (!amdgcn.vgpr_range<[? + 4]>, !amdgcn.vgpr_range<[? + 4]>) -> !amdgcn.vgpr_range<[? + 4]>
// CHECK:           %[[TEST_INST_1:.*]] = amdgcn.test_inst outs %[[MAKE_REGISTER_RANGE_1]] ins %[[TEST_INST_0]] : (!amdgcn.vgpr_range<[? + 4]>, !amdgcn.vgpr_range<[? + 4]>) -> !amdgcn.vgpr_range<[? + 4]>
// CHECK:           %[[TEST_INST_2:.*]] = amdgcn.test_inst outs %[[TEST_INST_0]] ins %[[TEST_INST_1]] : (!amdgcn.vgpr_range<[? + 4]>, !amdgcn.vgpr_range<[? + 4]>) -> !amdgcn.vgpr_range<[? + 4]>
// CHECK:           %[[TEST_INST_3:.*]] = amdgcn.test_inst outs %[[TEST_INST_1]] ins %[[TEST_INST_2]] : (!amdgcn.vgpr_range<[? + 4]>, !amdgcn.vgpr_range<[? + 4]>) -> !amdgcn.vgpr_range<[? + 4]>
// CHECK:           %[[TEST_INST_4:.*]] = amdgcn.test_inst outs %[[TEST_INST_2]] ins %[[TEST_INST_3]] : (!amdgcn.vgpr_range<[? + 4]>, !amdgcn.vgpr_range<[? + 4]>) -> !amdgcn.vgpr_range<[? + 4]>
// CHECK:           return %[[TEST_INST_3]] : !amdgcn.vgpr_range<[? + 4]>
// CHECK:         }
