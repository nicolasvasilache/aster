...
amdgcn.module @mfma_f8f6f4_mod target = #amdgcn.target<gfx950> isa = #amdgcn.isa<cdna4> {
  ...
  func.func private @init_vgprx8(%cst: i32) -> (!amdgcn.vgpr<[? + 8]>)
  ...
  amdgcn.kernel @scaled_mfma_nonidentity_scales arguments <[
    ...
    %a = func.call @init_vgprx8(%fp8_175) : (i32) -> (!amdgcn.vgpr<[? + 8]>)

    // B: FP8 E4M3 1.25 = 0x3A, packed 4 per dword: 0x3A3A3A3A = 976894522
    %fp8_125 = arith.constant 976894522 : i32
    %b = func.call @init_vgprx8(%fp8_125) : (i32) -> (!amdgcn.vgpr<[? + 8]>)
  ...
  amdgcn.kernel @scaled_mfma_split_k_accum arguments <[
    ...
    %a_lo = func.call @init_vgprx4(%fp8_10) : (i32) -> (!amdgcn.vgpr<[? + 4]>)

    // A upper half (k=64..127): FP8 E4M3 1.5 = 0x3C, packed: 0x3C3C3C3C = 1010580540
    %fp8_15 = arith.constant 1010580540 : i32
    %a_hi = func.call @init_vgprx4(%fp8_15) : (i32) -> (!amdgcn.vgpr<[? + 4]>)

    // Combine into 8-VGPR A range
    %a0:4 = amdgcn.split_register_range %a_lo : !amdgcn.vgpr<[? + 4]>
    %a1:4 = amdgcn.split_register_range %a_hi : !amdgcn.vgpr<[? + 4]>
    ...
    %b_lo = func.call @init_vgprx4(%fp8_20) : (i32) -> (!amdgcn.vgpr<[? + 4]>)

    // B upper half (k=64..127): FP8 E4M3 0.5 = 0x30, packed: 0x30303030 = 808464432
    %fp8_05 = arith.constant 808464432 : i32
    %b_hi = func.call @init_vgprx4(%fp8_05) : (i32) -> (!amdgcn.vgpr<[? + 4]>)

    // Combine into 8-VGPR B range
    %b0:4 = amdgcn.split_register_range %b_lo : !amdgcn.vgpr<[? + 4]>
    %b1:4 = amdgcn.split_register_range %b_hi : !amdgcn.vgpr<[? + 4]>
