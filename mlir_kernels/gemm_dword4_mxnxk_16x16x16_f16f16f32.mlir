  ...
  func.func private @matmul_loop(
    ...
    scf.for %k = %c0 to %K step %c1 {
      ...
      scf.for %d_mmkk = %c0 to %d_MMKK step %c1 {
        ...
        %pos_desc_a = aster_utils.struct_create(%a_global, %i_pos, %k_pos, %GLOBAL_STRIDE_IN_BYTES, %ii_pos, %kk_pos, %elt_size) : (!sx2, index, index, index, index, index, index) -> !tensor_position_descriptor_2level_2d
        func.call @global_load_to_lds_wave_16x16_f16_wait(%pos_desc_a, %lds_a_base_off, %LDS_STRIDE_IN_BYTES) : (!tensor_position_descriptor_2level_2d, index, index) -> ()
      } {aster.constexpr}
    ...
    scf.for %d_mmnn = %c0 to %d_MMNN step %c1 {
      ...
      %pos_desc_c = aster_utils.struct_create(%c_global, %i_pos, %j_pos, %GLOBAL_C_STRIDE_IN_BYTES, %ii_pos, %jj_pos, %elt_size_c) : (!sx2, index, index, index, index, index, index) -> !tensor_position_descriptor_2level_2d
      %true = arith.constant true
      func.call @global_store_wave_16x16xf32_C_fragment_wait(%fragment, %pos_desc_c, %true) : (!vx4, !tensor_position_descriptor_2level_2d, i1) -> ()
    } {aster.constexpr}
